...
SELECT s.State 
AS State_Name,COUNT(DISTINCT cr.`Constituency ID`) AS Total_Seats_Available
...
SELECT 
    cr.`Winning Candidate` AS WinningCandidate,
    p.Party AS Party,
    p.party_alliance AS PartyAlliance,
    cr.`Total Votes` AS TotalVotes,
    cr.Margin AS Margin,
    cr.`Constituency Name` AS ConstituencyName,
    s.State AS State
FROM constituencywise_results cr
...
SELECT 
    ...
    SUM(CASE WHEN p.party_alliance = 'NDA' THEN 1 ELSE 0 END) AS NDASeatsWon,
    SUM(CASE WHEN p.party_alliance = 'I.N.D.I.A' THEN 1 ELSE 0 END) AS INDIASeatsWon,
    SUM(CASE WHEN p.party_alliance = 'OTHER' THEN 1 ELSE 0 END) AS OTHERSeatsWon
FROM 
    constituencywise_results cr
JOIN 
    partywise_results p ON cr.`Party ID` = p.`Party ID`
...
JOIN (
        SELECT 
            `Constituency ID`,
            MAX(`EVM Votes`) AS MaxVotes
        FROM constituencywise_details
        GROUP BY `Constituency ID`
     ) AS t
     ON cd.`Constituency ID` = t.`Constituency ID`
    AND cd.`EVM Votes` = t.MaxVotes
...
SELECT 
    COUNT(DISTINCT cr.`Constituency ID`) AS TotalSeats,
    COUNT(DISTINCT cd.Candidate) AS TotalCandidates,
    COUNT(DISTINCT p.Party) AS TotalParties,
    SUM(cd.`EVM Votes` + cd.`Postal Votes`) AS TotalVotes,
    SUM(cd.`EVM Votes`) AS TotalEVMVotes,
    SUM(cd.`Postal Votes`) AS TotalPostalVotes
FROM 
    constituencywise_results cr
JOIN 
    constituencywise_details cd 
